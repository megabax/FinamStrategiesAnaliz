...
**Вежливое использование:** не перегружайте серверы Finam/comon частыми и параллельными запросами. Между обращениями вставляются **случайные паузы** (uniform между MIN и MAX из `.env`, см. `.env.example`: `COMON_API_PAUSE_*`, `COMON_SELENIUM_*`, `COMON_STRATEGY_PAUSE_*`; `COMON_PAUSES_ENABLED=false` — отключить). Не запускайте массовую выгрузку без необходимости, предпочитайте инкрементальное обновление. Соблюдайте правила сайта; используйте проект разумно и на свой риск.
